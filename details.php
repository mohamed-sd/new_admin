...
<!-- الحقول الجديدة -->
<div class="field"><span>👷 عطل تشغيل:</span> <?= htmlspecialchars($data['hr_fault']) ?></div>
<div class="field"><span>🔧 عطل صيانة:</span> <?= htmlspecialchars($data['maintenance_fault']) ?></div>
<div class="field"><span>⛏️ عطل حفار:</span> <?= htmlspecialchars($data['excavator_fault']) ?></div>
<div class="field"><span>➕ ساعات أخرى:</span> <?= htmlspecialchars($data['other_hours']) ?></div>
<div class="field"><span>⏳ إجمالي التوقف:</span> <?= htmlspecialchars($data['total_downtime_hours']) ?></div>
