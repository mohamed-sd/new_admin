...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit'])) {
  $table = isset($_POST['table']) ? $_POST['table'] : 'tipper';
...
<body>
  ...
  <div class="main" id="main">
    ...
    <div class="container">
      <h2>إضافة بيانات قلاب (Tipper)</h2>

      <?php if ($message)
        echo $message; ?>

      <form action="" method="POST" novalidate>
        <input type="hidden" name="table" value="tipper">
        ...
        <div class="grid">
          ...
          <input type="number" name="counter_start" value="<?php echo old('counter_end', 0); ?>">
        </div>
          ...
          <div>
            ...
            <input type="text" name="counter_diff" id="counter_diff" readonly>
          </div>

             <div>
          <label> عدد النقلات</label>
          <input type="number" name="number_of_moves" value="<?php echo old('number_of_moves', 0); ?>">
        </div>
...
function calculateDiff() {
  let counter_start = parseFloat(document.querySelector("input[name='counter_start']").value) || 0;
  let counter_end = parseFloat(document.querySelector("input[name='counter_end']").value) || 0;

  let diff = counter_end - counter_start;
  if (diff < 0) diff = 0; // حماية من السالب

  document.getElementById("counter_diff").value = diff;
}

// شغل الحساب عند أي تغيير في البداية أو النهاية
document.querySelectorAll("input[name='counter_start'], input[name='counter_end']")
